...
// стр 159
function calculate3ByteValue(mid, val, { kti, ktu }) {
...
function getPollItems(mid, period, ntariffs, meterType) { 
  switch (mid) {
    ...
    case "E":
      // return getReq05(ntariffs, period, 0x00); // всего от сброса = 0
      return getReq05(ntariffs, period, 0x40); // за текущие сутки
    ...
    default: throw {message:'getPollItems: Unknown mid = '+mid};
  }
}
...
function getReq05(ntariffs, count, byte2) {
...
function getErrorTxt(byte) {
    switch (byte) {
        case 0x00: return 'OK';
        case 0x01: return 'Недопустимая команда или параметр';
        case 0x02: return 'Внутренняя ошибка счетчика';
        case 0x03: return 'Недостаточен уровень доступа';
        case 0x04: return 'Внутренние часы уже корректировались в течение текущих суток';
        case 0x05: return 'Не открыт канал связи';
        case 0x06: return 'Повторить запрос в течение 0.5 сек';
        case 0x07: return 'Не готов результат измерения или Нет данных по запрашиваемому параметру';
        case 0x08: return 'Счетчик занят';
        default: return 'Не распознанная ошибка. Байт состояния='+byte.toString(16);
    }
}