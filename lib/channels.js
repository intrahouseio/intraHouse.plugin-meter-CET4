...
function energyChannels(ntariffs, sut='') {
    ...
    ['EAP'+sut,'EAM'+sut,'ERP'+sut,'ERM'+sut].forEach(item => {
        for (let t=0; t<=ntariffs; t++) {
            result.push({id:item+(t ? 'T'+t : ''), note: nameOfEnergy(item, t, sut), desc: "SensorA" });
        }
    });
    return result;
}

function nameOfEnergy(eid, t, sut) {
    let tstr = (t ? ' тариф '+t : '');
    let sutstr = (sut ? ' за сутки ' : ' всего ');
    switch (eid) {
        case 'EAP': return 'Энергия A+ '+sutstr+tstr;
        case 'EAM': return 'Энергия A- '+sutstr+tstr;
        case 'ERP': return 'Энергия R+ '+sutstr+tstr;
        case 'ERM': return 'Энергия R- '+sutstr+tstr;
        default:
    }
}
